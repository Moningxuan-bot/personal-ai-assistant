...
# DeepSeek V4 角色沉浸 marker — 附加在首轮 user 消息末尾
# 来源：victorchen96/deepseek_v4_rolepaly_instruct
# 训练时注入位置，后续每轮自动生效。让 DS 在 <think> 内做第一人称内心独白。
ROLEPLAY_INNER_OS_MARKER = """
...
# 无活跃对话时，为记账事件注入的 RP 锚点
_SPENDING_RP_ANCHOR = f"阿玖，接下来你都按你自己来。{ROLEPLAY_INNER_OS_MARKER}"
...
class ChatService:
    ...
    async def chat(
        ...
        # 3. 检查是否为此会话的首轮消息 → 注入 RP 沉浸 marker
        existing_count = await self._count_messages(conv.id)
        if existing_count == 0:
            user_message = user_message + ROLEPLAY_INNER_OS_MARKER

        # 4. Save user message（含 marker 的首轮消息也会存储，后续轮次自动生效）
        user_msg = Message(
    ...
    async def respond_to_spending(
        ...
        # 新会话 → 注入 RP 锚点，确保后续聊天有角色沉浸 marker
        msg_count = await self._count_messages(conv.id)
        if msg_count == 0:
            anchor = Message(
                conversation_id=conv.id,
                role="user",
                content=_SPENDING_RP_ANCHOR,
            )
            self.db.add(anchor)
            await self.db.commit()
